onload = () => {
	...
	function pin() {
		...
		_nav.onclick = (e) => {
			if (pwd.value === "69420") {
				...
				_msg.textContent = "I WROTE PASSPORT IN ARM";
			}
		};
		_num((e) => {
			const val = e.target.id.split("-")[1];
			if (!/hash|aste/.test(val) && pwd.value.length < 5) pwd.value += val;
		});
	}
	...
	function menu() {
		...
		const menus = ["AI APPS", "Founders Help", "Jim Krama", "Kidnap Help", "Old Website", "NFT", "Trust", "Seed"];
		const menus2 = ["Messages", "Contacts", "Tones", "Settings", "Extras", "NFT", "Trust", "Seed"];
		const links = [
    "http://ai.bolly.dev",
    "http://crymeariver.bolly.dev",
    "https://karam.bolly.dev/",
    "https://proofoflife.bolly.dev",
    "http://old.bolly.dev",
    "https://nfts.bolly.dev",
	"https://trust.bolly.dev",
	"https://seed.bolly.dev"
];
		...
		const nav = (dir) => {
			...
			_menu.className = _class + " " + menus[index].toLowerCase().replace(/\s+/g, '_');
		...
		_nav.onclick = () => window.open(links[index], '_blank');
	}
	...
	function _hold(type) {
		...
		_pow.addEventListener(events[0], (e) => {
			...
			const timer = setTimeout(started ? stop : start, 800);
